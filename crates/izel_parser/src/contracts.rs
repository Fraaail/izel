...
impl ContractChecker {
    ...
    pub fn check_requires_raw(name: &str, params: &[crate::ast::Param], requires: &[crate::ast::Expr], args: &[ConstValue], call_span: Span) -> Vec<Diagnostic> {
    ...
    pub fn check_ensures_raw(name: &str, ensures: &[crate::ast::Expr], ret_val: &ConstValue, ret_span: Span, params: &HashMap<String, ConstValue>) -> Vec<Diagnostic> {
        let mut diagnostics = Vec::new();
...
mod tests {
    ...
    fn test_temporal_constraints() {
        ...
        let forge = ast::Forge {
            name: "test".to_string(),
            generic_params: vec![],
            params: vec![ast::Param { name: "n".to_string(), ty: ast::Type::Prim("i32".to_string()), span: Span::dummy() }],
        ...
        let diags = ContractChecker::check_ensures(&forge_ens, &ConstValue::Int(1), Span::dummy(), &HashMap::new());
        assert!(diags.is_empty());

        // 4. Check ensures(result > 0) with result = 0 (Failure)
        let diags = ContractChecker::check_ensures(&forge_ens, &ConstValue::Int(0), Span::dummy(), &HashMap::new());
        assert!(!diags.is_empty());
