...
- Added concrete compile-pass/compile-fail fixtures replacing placeholder files.
- Added broader `izel_pm` parser and CLI branch coverage tests.

### Changed
- Replaced placeholder dual round-trip test body generation with an empty, valid body.
- Type checker now records inferred expression types in `expr_types`.
- MIR codegen now emits LLVM phi handling instead of no-op placeholder behavior.
